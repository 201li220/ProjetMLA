...
class Adapter(nn.Module):
    def __init__(self,input_size, adapter_size):
        ...
        self.FeedForward_down_layer = nn.Linear(input_size, adapter_size)
        self.FeedForward_up_layer = nn.Linear(adapter_size,input_size)
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.dropout)
        self.adapter1 = Adapter(config.emb_size, adapter_size = 256)
        self.LayerNorm = nn.LayerNorm(config.emb_size, eps=config.layer_norm_eps)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.dropout)
        self.adapter2 = Adapter(config.emb_size, adapter_size = 256)
        self.LayerNorm = nn.LayerNorm(config.emb_size, eps=config.layer_norm_eps) 
